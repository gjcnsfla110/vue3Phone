...
import {goodsReviewList,createGoodsReview,deleteGoodsReview} from "@/api/review/goodsReview.js";
...
const {
  ...
  getList: goodsReviewList,
  delete:deleteGoodsReview,
});
const {
  formDrawerRef,
  formRef,
  ...
  beforeSubmit:(form)=>{
    form.goods_id=goodsId.value;
    return form;
  },
  getDataList:getData,
  create:createGoodsReview,
});
getData();
const goodsId = ref(props.goodsId);
// 1. 단일 props 감시 (가장 많이 씀!)
watch(() => props.goodsId, (newId, oldId) => {
      searchForm.goods_id = newId;
      goodsId.value = newId;
      getData();
})
//상세화면 보기
