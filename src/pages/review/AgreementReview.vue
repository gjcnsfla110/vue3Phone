...
import {agreementReviewList,deleteAgreementReview,createAgreementReview} from "@/api/review/agreementReview.js";
...
const {
  ...
  getList: agreementReviewList,
  delete:deleteAgreementReview,
});
const {
  formDrawerRef,
  formRef,
  ...
  beforeSubmit:(form)=>{
    form.agreement_id = agreement_id.value;
    return form;
  },
  getDataList:getData,
  create:createAgreementReview,
});
getData();
const agreement_id = ref(props.agreementId);
// 1. 단일 props 감시 (가장 많이 씀!)
watch(() => props.agreementId, (newId, oldId) => {
  searchForm.agreement_id = newId;
  agreement_id.value = newId;
  getData();
})
//상세화면 보기
