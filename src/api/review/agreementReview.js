...
export function agreementReviewList(page,limit=10,query={}) {
    let q = queryUrl(query);
    let is = q ? '&':'?';
    return service.get(`admin/agreement/review/${page}/list${q}${is}limit=${limit}`);
}

export function createAgreementReview(data) {
    // 1. 파일 추가 (여러 개 가능)
    ...
    return service.post(`admin/agreement/review/create`,formData,{
        headers: {
            'Content-Type': 'multipart/form-data'
        }});
}

export function deleteAgreementReview(id){
    return service.post(`admin/agreement/review/${id}/delete`);
}
