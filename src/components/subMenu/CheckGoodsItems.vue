<script setup>
    ...
    //상품 리스트갖고오는 함수
    const getData = ()=>{
      checkGoodsList(currentPage.value,limit.value,{sideCategory_id:searchCategoryId.value}).then((res)=>{
        dataList.value=res.list;
        total.value = res.total;
        menulist.value = menuListTrees(res.mainCategory.filter((item)=>item.category_id ===0),res.mainCategory,'category_id');
      ...
      <el-container>
        ...
        <el-main>
            ...
            >
              ...
              <el-table-column label="상품타입" width="180" prop="type"></el-table-column>
              <el-table-column label="원가격" width="180" align="center">
                <template #default="{row}">
                   {{priceDollar(row.price)}}원
                </template>
              </el-table-column>
              <el-table-column label="판매가격" width="180" align="center">
                <template #default="{row}">
                  <span v-if="row.type !== '二手商品'">{{priceDollar(row.price1)}}원</span>
                  <span v-else>{{priceDollar(row.price2)}}원</span>
                </template>
              </el-table-column>
